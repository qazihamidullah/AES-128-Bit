...
void setup()
{
  Serial.begin(4800);   //setting baud rate
  uint8_t i, r;      //declaring variables
...
*/
  ...
  Serial.setTimeout(5000);    //so that serial.parseint () doesnot finish immediately
  Serial.println("Enter Your Key(16 digit only): ");
   uint8_t key[16];// temp[16];
  while(Serial.available() == 0)    //gets the stored bytes from tbe serial port
  { }
  for(int i=0; i<16; i++)
  {
    
    key[i] = Serial.parseInt();        //taking input fom user
    //key[16] = Serial
  }
  ...
  */
  ...
  { 
    ...
    message.toCharArray(plaintext, 16);
  }
  ...
  for (i = 0; i < AES_BLOCK_SIZE; i++) {
    Serial.println(plaintext[i], HEX);
  }
  Serial.println("\n\n"); 
  ...
  for ( r = 0; r <= AES_ROUNDS; r++ ) {
    for (i = 0; i < AES_BLOCK_SIZE; i++) {
      Serial.println(roundkeys[r*AES_BLOCK_SIZE+i], HEX);
    }
    Serial.println("\n");
  }
  Serial.println("\n");
  ...
  for (i = 0; i < AES_BLOCK_SIZE; i++) {
    Serial.println(ciphertext[i], HEX);
  }
  for (i = 0; i < AES_BLOCK_SIZE; i++) {
    if ( ciphertext[i] != const_cipher[i] ) { break; }
  }
  if ( AES_BLOCK_SIZE != i ) { Serial.println("\nENCRYPT WRONG\n\n"); }
  else { Serial.println("\nENCRYPT CORRECT\n\n"); }
